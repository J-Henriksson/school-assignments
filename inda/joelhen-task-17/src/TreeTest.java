 ...
 *
 * @author Simon Larsén Joel Henriksson
 * @version 2024-02-16
 */
public class TreeTest {
    ...
    public void leavesIsZeroWhenTreeIsEmpty() {
        ...
        // Assert
        assertEquals(0, emptyTree.leaves());
    }
    ...
    public void leavesIsCorrectWhenTreeIsPerfect() {
        ...
        int[] elementsInPerfectTree = new int[] {8, 3, 10, 1, 6, 9, 14};
        for (int elem : elementsInPerfectTree) {
            perfectTree.insert(elem);
        }

        int nodes = 7; 
        int expectedLeaves = (nodes + 1) / 2; 
        int actualLeaves = perfectTree.leaves();

        assertThat(actualLeaves, equalTo(expectedLeaves));
    }
    ...
    public void insertValuesInAscendingOrderIncrementsHeight() {
        // Arrange
        Tree<Integer> ascendingOrder = new Tree<>();
        int[] ascendingOrderArray = new int[] {20, 15, 9, 7, 5, 4, 2};
        for (int i = 0; i < ascendingOrderArray.length; i++) {
            ascendingOrder.insert(ascendingOrderArray[i]);
            // Assert
            assertThat(ascendingOrder.height(), equalTo(i));
        }
    }

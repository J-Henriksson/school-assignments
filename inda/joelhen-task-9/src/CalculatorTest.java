...
public class CalculatorTest {
    ...
    @Test
    public void inverseGivesCorrectResultForPositiveValues() {
        double expected = 0;
        for (double i = 1; i <= 10; i++) {
            calculator.add(i);
            expected =  1/ i;
            assertEquals(expected, calculator.inverse(), ALLOWED_ERROR);
            calculator.reset();
        }
    }

    @Test
    public void inverseGivesCorrectResultForNegativeValues() {
        double expected = 0;
        for (double i = -1; i >= -10; i--) {
            calculator.add(i);
            expected =  1/ i;
    ...
    @Test(expected = IllegalArgumentException.class)
    public void inverseThrowsIllegalArgumentExceptionWhenValueIsZero() {;  // Set the value to zero
        calculator.inverse();
    }

    @Test 
    public void inverseTwiceLeavesValueUnchanged() {
        double expected;
        for (double i = 1; i <= 10; i++) {
            calculator.add(1);
            expected = i;
            calculator.inverse();
            assertEquals(i, calculator.inverse(), ALLOWED_ERROR);
        }
    }
